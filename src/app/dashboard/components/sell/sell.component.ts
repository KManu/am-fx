...
export class SellComponent implements OnInit {
  ...
  formInit() {
    ...
    const a = this.sellGHSForm.get('currency').valueChanges
      .subscribe(value => {
        this.selectedCurrency = this.currencies.find(e => e.code === value);
        this.selectedRate = this.selectedCurrency.selling_rate;
      });
  ...
  submitForm(formValues) {
    ...
    this.storage.getItem<Users>('am-user', { schema: userSchema })
      .subscribe(user => {
        ...
        formValues['rate'] = this.selectedCurrency.selling_rate;
